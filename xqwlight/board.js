...
Board.prototype.response = function () {
    ...
    setTimeout(function () {
        ...
        if (this_.search.info) {
            console.log(this_.search.info);
            this_.search.info = null
        }
        setTimeout(() => localStorage.setItem("pos", board.pos.toFen(), 400));
    }, 250);
};
...
Board.prototype.hint = function () {
    if (this.busy || this.result != RESULT_UNKNOWN || this.search == null) {
        return;
    }
    this.thinking.style.visibility = "visible";
    this.busy = true;
    setTimeout(() => {
        ...
        if (hintMove > 0) {
            ...
            var blink = setInterval(function () {
                if (blinkCount % 2 === 0) {
                    this_.drawSquare(sqSrc, true);
                    this_.drawSquare(sqDst, false);
                } else {
                    this_.drawSquare(sqSrc, false);
                    this_.drawSquare(sqDst, true);
                }
                blinkCount++;
